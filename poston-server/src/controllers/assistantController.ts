...
export const askGPT = async (req: Request, res: Response) => {
  try {
    ...
    const answer = response.data.choices?.[0]?.message?.content || 'Извините, нет ответа.'
    return res.json({ answer })
  } catch (e: any) {
    // Если OpenAI вернул ошибку, пересылаем сообщение об ошибке
    if (e.response && e.response.data) {
      return res.status(500).json({ message: 'GPT Error: ' + JSON.stringify(e.response.data) })
    }
