...
export const createPurchase = async (req: Request, res: Response) => {
  try {
    ...
    if (dt === 'BRANCH') {
      ...
      // Автоматически подбираем свободный слот под размеры товара
      const slots = await Slot.findAll({ where: { postomat_id: postomat_id } })
      // Находим слот, в котором нет активного заказа и который подходит по размеру
      ...
      // Курьер будет назначен администратором позже
      courier_id = null
    } else {
    ...
    const newPurchase = await Purchase.create({
      ...
      date_buy: new Date(),
      // Для branch и postomat предположим, что товар сразу доставлен в точку выдачи
...
export const assignCourier = async (req: Request, res: Response) => {
  try {
    ...
    // Получаем обновленный заказ с данными
    ...
    // Оповещаем курьера через Telegram, если возможно
...
export const markDelivered = async (req: Request, res: Response) => {
  try {
    ...
    // Проверка прав: админ может всегда, курьер только для своих заказов
    ...
    // Отмечаем доставленным
    await Purchase.update(
      { date_receive: new Date() },
      { where: { id: purchase.id } }
    )
    // Отправляем уведомление покупателю через Telegram, если привязан
